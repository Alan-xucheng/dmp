...
class Upload {
	...
	function do_upload($field = 'userfile')
	{
		...
		$this->file_name = strtolower($this->_prep_filename($_FILES[$field]['name']));
		...
		{
			if ( ! @move_uploaded_file($this->file_temp, $this->upload_path.strtolower($this->file_name)))
			{
	...
	function is_allowed_filetype()
	{
		...
		$package_types = array('apk', 'ipa');
		$tmp_array = explode(".", $this->file_name);
		$types = "";
		if(is_array($tmp_array))
		{
			$types = strtolower($tmp_array[1]);
		}
